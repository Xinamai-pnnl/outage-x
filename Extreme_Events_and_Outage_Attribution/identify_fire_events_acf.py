...
# ==== USER SETTINGS ====
INPUT_DIR = "./outputs/fire/county_fire_binary_thrshld5"
OUTPUT_DIR = "./events/fire/event_fire_thrshld5"
OUTPUT_DIR_acf = "./events/fire/event_fire_thrshld5_acf"
os.makedirs(OUTPUT_DIR, exist_ok=True)  
os.makedirs(OUTPUT_DIR_acf, exist_ok=True)  
FILE_GLOB = "*_binary_fire.csv"  
TIME_COL = "timestamp"
FLAG_COL = "fire"  # Binary column (0/1 indicating exceedance)
...
def county_from_filename(path: str) -> str:
    m = re.match(r"^(\d{4,5})_binary_fire\.csv$", os.path.basename(path))  # Updated regex for binary files
    if not m:
...
def process_county(path: str) -> Optional[dict]:
    ...
    try:
        ...
        out_path = os.path.join(OUTPUT_DIR, f"fire_events_{county_fips}.csv")
